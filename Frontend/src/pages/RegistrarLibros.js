...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, FlatList, Image, Modal, Alert, ScrollView } from 'react-native';
...
const LibraryScreen = () => {
  ...
  const validateFields = () => {
    if (!title || !author || !image) {
      Alert.alert('Error', 'Por favor, complete los campos obligatorios: Título, Autor e Imagen');
      return false;
    }
    return true;
  };
  const confirmDeleteBook = (id) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Eliminar', onPress: () => handleDeleteBook(id), style: 'destructive' },
      ]
    );
  };
      ...
      >
        <View style={styles.modalContainer}>
          <ScrollView contentContainerStyle={styles.modalContent}>
            <Text style={styles.modalTitle}>{selectedBook ? 'Modificar Libro' : 'Ingresar Libro'}</Text>
            ...
            <TextInput
              ...
              />
            <TextInput style={styles.input} placeholder="Categorías" value={categories} onChangeText={setCategories} />
            <TextInput style={styles.input} placeholder="Maturidad" value={maturity} onChangeText={setMaturity} />
            <TextInput style={styles.input} placeholder="Enlace de previsualización" value={previewLink} onChangeText={setPreviewLink} />
            <TextInput style={styles.input} placeholder="Stock" value={stock.toString()} keyboardType="numeric" onChangeText={handleStockChange} />
            <TouchableOpacity style={styles.submitButton} onPress={selectedBook ? handleSaveEditBook : handleAddBook}>
              <Text style={styles.buttonText}>{selectedBook ? 'Guardar Cambios' : 'Agregar Libro'}</Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  editButton: { backgroundColor: '#FFD700', padding: 5, borderRadius: 5, width: '48%', alignItems: 'center' },
  deleteButton: { backgroundColor: '#FF6347', padding: 5, borderRadius: 5, width: '48%', alignItems: 'center' },
  ...
  pagination: { textAlign: 'center', fontSize: 16, color: '#5D4037', marginVertical: 10 },
  modalContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)' },
  modalContent: { backgroundColor: 'white', padding: 20, borderRadius: 10, width: 350, marginTop: 80,},
  modalTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 15, textAlign: 'center', color: '#5D4037' },
  input: { height: 40, borderColor: '#ccc', borderWidth: 1, marginBottom: 10, paddingLeft: 10, borderRadius: 5 },
  submitButton: { backgroundColor: '#000000', padding: 10, borderRadius: 5, alignItems: 'center', marginTop: 10 },
  closeButton: { backgroundColor: '#BDBDBD', padding: 10, borderRadius: 5, alignItems: 'center', marginTop: 10 },
